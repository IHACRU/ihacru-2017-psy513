...
# ---- inspect-data-2 -----------------------------------------------------------
# varnames <- c(cerner_address_keys, data_warehouse_address, "unit_name",compressor_names, program_classes, palette_colours)
...
# ---- inspect-data -----------------------------------------------------------
ds_location_map %>% 
  dplyr::group_by_("site_key", "facility_key", "building_key","unit_key", 
                   "location_category", "location_grouping", "location_type",
                   "unit_name" ) %>% 
  dplyr::mutate(n_address = n()) %>% dplyr::ungroup() %>% 
  dplyr::group_by_("intensity_type", "intensity_severity_risk", "clinical_focus",
                   "service_type","service_location","population_age", "provider_mix") %>% 
  dplyr::mutate(n_class = n()) %>% dplyr::ungroup() %>% 
  dplyr::mutate(
    n_address = as.character(n_address),
    n_class   = as.character(n_class), 
    location_class_code = as.character(location_class_code),
  ...
  DT::datatable( 
    class   = 'cell-border stripe',
    caption = "Map of Cross-Continuum Classification of VIHA's service system",
    filter  = "top", 
    options = list(
      pageLength = 10, 
      autoWidth  = TRUE
