...
path_input_patient_events <-  "./data-unshared/derived/dto_patient_events_addictions_4067.rds" # research cohort
...
ds %>% distinct(location_mapping_id) %>% count() %>% neat()
...
# ---- publish ---------------------------------------
path_report_1 <- "./sandbox/addictions-4067/addictions-4067-summative.Rmd"
...
print_encounter_timeline <- function(
  ...
  # # person_id_ <- 329656
  # # person_id_ <- 413785
  # path_out_folder <- "./sandbox/mhsu-74/prints/"
    ...
    for(i in ids){
      ...
      ds %>% 
        print_encounter_timeline(
          person_id_ = i,
          color_by_ = col,
          path_out_folder =  paste0("./sandbox/mhsu-74/prints/"),  
          prefex = "mhsu74"
        )
    }
  }
...
path_folder <- paste0("./sandbox/mhsu-74/prints/")
...
# saved these counts for Ken to use 2016-11-28
readr::write_csv(print_types,"./reports/06-service-types-updated/type-counts.csv")
